...
function getLine(elem, key, run_data, run) {
...
function getLines(run, container_id, keys, run_data) {
    for (let i = 0; i < all_run_keys.length; i++) {
        ...
        emptyOrCallback(keys, interrupt_hide_zero_na_graphs, getLine, elem, value, run_data, run);
    }
}
...
function interrupts(hide: boolean) {
    ...
    if (got_interrupt_data && (hide == interrupt_hide_zero_na_graphs || allRunCPUListUnchanged(interrupt_cpu_list))) {
        return;
    }
